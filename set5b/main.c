...
void test1(void) {
    ...
    CsvReader *reader = csv_open("test1.csv");
...
void test2(void) {
    char *data[5][4] = {{"Pac-Man", "1980", "One, two, three", "150"},
...
void test3(void) {
    char *data[3][1] = {{"all, your, base, are, belong, to, us"},
                        {"\"all\" \"your\" \"base\""},
                        {"all\nyour\nbase\nare\nbelong\nto\nus"}};

    CsvReader *reader = csv_open("test3.csv");
    test_assert(reader != NULL);
    CsvRecord *record;
    int count = 0;
    while (record = csv_next_record(reader)) {
        test_assert(record->count == 1);
        for (int i = 0; i < 1; i++) {
            test_assert(strcmp(record->fields[i], data[count][i]) == 0);
        }
        csv_free_record(record);
